...
# Mixed-effects logistic regression with random intercepts for Case_ID and Rater.
# Configurable via configs/experiment.yaml.
...
main <- function() {
  ...
  # Optional renaming to canonical labels
  if (!is.null(rename_map) && length(rename_map) > 0) {
    cn <- names(df)
    for (k in names(rename_map)) {
      cn <- ifelse(grepl(k, cn, ignore.case = TRUE), rename_map[[k]], cn)
    }
    names(df) <- cn
  }

  stopifnot(all(c("case_id") %in% names(df)))
  stopifnot(all(juniors %in% names(df)),
            all(seniors %in% names(df)),
            all(ai_models %in% names(df)))

  df <- df %>% mutate(
    juniors_mean = rowMeans(across(all_of(juniors)), na.rm = TRUE),
  ...
  df_long$group <- factor(
    df_long$group,
    levels = c("Senior","Junior", ai_models)
  )

  m_bin <- lme4::glmer(correct ~ group + (1|case_id) + (1|rater),
  ...
  se <- sqrt(diag(vcov(m_bin)))
  OR <- exp(beta)
  lwr <- exp(beta - 1.96 * se)
  upr <- exp(beta + 1.96 * se)

  wald <- summary(m_bin)$coefficients
  pval <- wald[, "Pr(>|z|)"]
