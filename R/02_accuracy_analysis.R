...
# Friedman test across raters and paired Wilcoxon post-hoc with Holm adjustment.
# Also computes bootstrap 95% CIs for cohorts and AI models.
# Configurable paths & name normalization via configs/experiment.yaml.
...
normalize_names <- function(nms, rename_map) {
  # Clean to syntactic names but then map known variants to canonical labels
  cleaned <- nms |> janitor::make_clean_names()
  # Apply provided rename_map (vector or named list of patterns)
  if (!is.null(rename_map) && length(rename_map) > 0) {
    for (k in names(rename_map)) {
      pat <- k
      tgt <- rename_map[[k]]
      cleaned <- ifelse(grepl(pat, cleaned, ignore.case = TRUE), tgt, cleaned)
    }
  }
  cleaned
}
...
main <- function() {
  ...
  # Long format for Friedman
  long <- raw %>%
    pivot_longer(cols = all_of(c(juniors, seniors, ai_models)),
  ...
  pair_full <- long %>%
    rstatix::pairwise_wilcox_test(score ~ rater, paired = TRUE, p.adjust.method = "holm") %>%
    arrange(p.adj)

  print(pair_full)

  # Cohort means (humans only) vs AI models
  raw <- raw %>%
    mutate(juniors_mean = rowMeans(across(all_of(juniors)), na.rm = TRUE),
           seniors_mean = rowMeans(across(all_of(seniors)), na.rm = TRUE))

  cohort_long <- raw %>%
    pivot_longer(cols = all_of(c("juniors_mean", "seniors_mean", ai_models)),
                 names_to = "cohort", values_to = "score") %>%
    mutate(cohort = factor(cohort, levels = c("juniors_mean", "seniors_mean", ai_models)))

  pair_cohort <- cohort_long %>%
    rstatix::pairwise_wilcox_test(score ~ cohort, paired = TRUE, p.adjust.method = "holm") %>%
    arrange(p.adj)

  print(pair_cohort)

  # Bootstrap means with percentile CI
  set.seed(42)
  boot_mean <- function(x, i) mean(x[i], na.rm = TRUE)

  boot_table <- cohort_long %>%
    group_by(cohort) %>%
