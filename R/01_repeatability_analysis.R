...
# Computes ICC(2,1) and pairwise weighted Cohen's kappa for each sheet in an Excel file.
# Configurable via configs/experiment.yaml (keys: repeatability.excel_path).
...
main <- function() {
  ...
  for (sh in ai_sheets) {
    ...
    ic <- tryCatch(
      irr::icc(as.matrix(dat), model = "twoway", type = "agreement", unit = "single"),
    ...
    # Ordinalize for weighted kappa (levels 0, 0.5, 1)
    dat_ord <- lapply(dat, function(x) factor(as.numeric(x), levels = c(0, 0.5, 1), ordered = TRUE))
    raters <- names(dat_ord); K <- length(raters)
    k_vals <- c()
